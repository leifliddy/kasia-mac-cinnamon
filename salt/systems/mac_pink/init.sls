...
deploy_vconsole_conf:
  file.managed:
    ...
    - source: salt://systems/mac_pink/files/vconsole.conf
    - user:   root
    - group:  root
    - mode:   644
...
deploy_mac_term_conf:
  file.managed:
    - name:   /etc/profile.d/mac-term.sh
    - source: salt://systems/mac_pink/files/mac-term.sh
    - user:   root
    - group:  root
    - mode:   644
...
deploy_touchpad.sh:
  file.managed:
    - name:   /usr/local/bin/touchpad.sh
    - source: salt://systems/mac_pink/files/touchpad.sh
    - user:   root
    - group:  root
    - mode:   644
  ...
  file.managed:
    ...
    - names:
      - /usr/share/backgrounds/images/{{ wallpaper_login }}:
        - source: salt://systems/mac_pink/files/{{ wallpaper_login }}
      - /usr/share/backgrounds/images/{{ wallpaper_desktop }}:
        - source: salt://systems/mac_pink/files/{{ wallpaper_desktop }}
...
deploy_double_scaling_desktop_config:
  file.managed:
    ...
    - source: salt://systems/mac_pink/files/double-scaling.desktop
    - user:   {{ user }}
    - group:  {{ user }}
    - mode:   644
...
deploy_dconf_config:
  file.managed:
    ...
    - source: salt://systems/mac_pink/files/dconf_keyfile
    - user:   {{ user }}
    - group:  {{ user }}
    - mode:   644
...
mute_startup_chime:
  cmd.run:
    - name: printf "\x07\x00\x00\x00\x80" > /sys/firmware/efi/efivars/SystemAudioVolume-7c436110-ab2a-4bbb-a880-fe41995c9f82   
